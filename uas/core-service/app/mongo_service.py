...
Menangani seluruh operasi CRUD pada MongoDB.
Di-refactor dari app/mongo_service.py project lama.
Logika CRUD dipertahankan, hanya disesuaikan untuk REST API.
"""
...
def cari_semua() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Konversi ObjectId ke string agar bisa di-serialize JSON
        for item in data:
...
def cari_by_id(produk_id: int) -> Optional[Dict[str, Any]]:
    ...
    Args:
        produk_id: ID produk yang dicari.

    Returns:
        Dictionary produk jika ditemukan, None jika tidak.
...
def insert_satu(produk: Dict[str, Any]) -> Optional[int]:
    """Insert satu produk ke MongoDB.

    Args:
        produk: Dictionary data produk (harus memiliki field '_id').

    Returns:
        ID produk yang diinsert jika berhasil, None jika gagal.
...
def muat_dataset() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"File dataset tidak ditemukan di {DATASET_PATH}")
        return []
    except json.JSONDecodeError as e:
        logger.error(f"Format JSON tidak valid: {e}")
        return []